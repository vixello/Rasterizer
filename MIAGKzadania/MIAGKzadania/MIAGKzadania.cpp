...
// Directional light parameters:
    DirectionalLight directionalLight(
        ...
        2.f,                        // Shininess (for Phong shading)
        .7f,                        // Intensity
    ...
    Spotlight spotlight(
        { -10.f, -1.f, 10.f },          // Position
        { 0.f, -5.f, 1.f },          // Direction vector (normalized)
        120.f,                       // Angle (in degrees)
        { .7f, .7f, .7f },          // Ambient color
    ...
    // Sphere
    float4x4 sphereTransform = Camera::Translation(0.2f, .2f, 1.f) * Camera::Scale(1.f, 1.f, 1.f) * Camera::RotationX(0);
    SphereMesh sphereGrid(vert, horiz, radius, buffer, projectionTransform * viewTransform * sphereTransform);
    sphereTransform = Camera::Translation(0.f, .2f, 1.f) * Camera::Scale(1.f, 1.f, 1.f) * Camera::RotationX(0);
    ...
    // Torus
    float4x4 torusTransform = Camera::Translation(-.2f, -0.1f, 0.f) * Camera::Scale(.4f, 0.4f, .4f) * Camera::RotationX(0) * Camera::RotationY(50);
    ...
    sphereGridGouraud.DrawGouraud(r, 0xFFA63446, 0xFFA63446, 0xFFA63446);
    ...
    torusGrid.DrawGouraud(r, 0xFF441151, 0xFF441151, 0xFF441151);
    /// Colors
