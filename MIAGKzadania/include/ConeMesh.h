...
private:
    ...
    void GenerateVertices() {
        ...
        for (int i = 0; i < segments_; ++i) {
            ...
            float u = 0.5f + 0.5f * cos(angle); // Map x coordinate to [0, 1]
            float v = 0.5f + 0.5f * sin(angle); // Map z coordinate to [0, 1]
            texCoords_.emplace_back(u, v);
        }

        // Center of the base circle
        vertices_.emplace_back(0, 0, 0);
        texCoords_.emplace_back(0.5f, 0.5f); // Texture coordinate for the center
    }
